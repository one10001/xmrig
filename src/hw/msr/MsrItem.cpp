...
pythonxm::MsrItem::MsrItem(const rapidjson::Value &value)
{
    if (!value.IsString()) {
        return;
    }
...
rapidjson::Value pythonxm::MsrItem::toJSON(rapidjson::Document &doc) const
{
    return toString().toJSON(doc);
}


pythonxm::String pythonxm::MsrItem::toString() const
{
