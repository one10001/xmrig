...
pythonxm::App::App(Process *process)
{
...
int pythonxm::App::exec()
{
    if (!m_controller->isReady()) {
        LOG_EMERG("no valid configuration found, try https://pythonxm.com/wizard");
...
void pythonxm::App::onConsoleCommand(char command)
{
    if (command == 3) {
...
void pythonxm::App::onSignal(int signum)
{
    switch (signum)
    {
    case SIGHUP:
