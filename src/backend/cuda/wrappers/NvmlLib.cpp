...
bool pythonxm::NvmlLib::init(const char *fileName)
{
    if (!m_initialized) {
        m_loader      = fileName;
        m_ready       = dlopen() && load();
...
const char *pythonxm::NvmlLib::lastError() noexcept
{
    return uv_dlerror(&nvmlLib);
}


void pythonxm::NvmlLib::close()
{
    if (m_ready) {
        pNvmlShutdown();
    }
...
bool pythonxm::NvmlLib::assign(std::vector<CudaDevice> &devices)
{
    uint32_t count = 0;
...
NvmlHealth pythonxm::NvmlLib::health(nvmlDevice_t device)
{
    if (!device) {
        return {};
    }
