...
constexpr uint32_t pythonxm::CudaAstroBWTRunner::BWT_DATA_STRIDE;


pythonxm::CudaAstroBWTRunner::CudaAstroBWTRunner(size_t index, const CudaLaunchData &data) :
...
bool pythonxm::CudaAstroBWTRunner::run(uint32_t startNonce, uint32_t *rescount, uint32_t *resnonce)
{
    return callWrapper(CudaLib::astroBWTHash(m_ctx, startNonce, m_target, rescount, resnonce));
}


bool pythonxm::CudaAstroBWTRunner::set(const Job &job, uint8_t *blob)
{
...
size_t pythonxm::CudaAstroBWTRunner::roundSize() const
{
...
size_t pythonxm::CudaAstroBWTRunner::processedHashes() const
{
