...
pythonxm::OclBaseRunner::OclBaseRunner(size_t id, const OclLaunchData &data) :
    m_ctx(data.ctx),
...
pythonxm::OclBaseRunner::~OclBaseRunner()
{
...
size_t pythonxm::OclBaseRunner::bufferSize() const
{
    return align(Job::kMaxBlobSize) + align(sizeof(cl_uint) * 0x100);
}


uint32_t pythonxm::OclBaseRunner::deviceIndex() const
{
    return data().thread.index();
}


void pythonxm::OclBaseRunner::build()
{
...
void pythonxm::OclBaseRunner::init()
{
...
cl_mem pythonxm::OclBaseRunner::createSubBuffer(cl_mem_flags flags, size_t size)
{
...
size_t pythonxm::OclBaseRunner::align(size_t size) const
{
    return VirtualMemory::align(size, m_align);
}


void pythonxm::OclBaseRunner::enqueueReadBuffer(cl_mem buffer, cl_bool blocking_read, size_t offset, size_t size, void *ptr)
{
...
void pythonxm::OclBaseRunner::enqueueWriteBuffer(cl_mem buffer, cl_bool blocking_write, size_t offset, size_t size, const void *ptr)
{
...
void pythonxm::OclBaseRunner::finalize(uint32_t *hashOutput)
{
