...
void pythonxm::FindSharesKernel::enqueue(cl_command_queue queue, size_t threads)
{
...
void pythonxm::FindSharesKernel::setArgs(cl_mem hashes, cl_mem shares)
{
...
void pythonxm::FindSharesKernel::setTarget(uint64_t target)
{
    setArg(1, sizeof(uint64_t), &target);
}


void pythonxm::FindSharesKernel::setNonce(uint32_t nonce)
{
