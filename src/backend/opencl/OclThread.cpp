...
pythonxm::OclThread::OclThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
bool pythonxm::OclThread::isEqual(const OclThread &other) const
{
...
rapidjson::Value pythonxm::OclThread::toJSON(rapidjson::Document &doc) const
{
