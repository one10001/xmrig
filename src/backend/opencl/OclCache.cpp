...
cl_program pythonxm::OclCache::build(const IOclRunner *runner)
{
...
#       ifdef _WIN32
        fileName = prefix() + "\\pythonxm\\.cache\\" + cacheKey(runner) + ".bin";
#       else
...
std::string pythonxm::OclCache::cacheKey(const char *deviceKey, const char *options, const char *source)
{
...
std::string pythonxm::OclCache::cacheKey(const IOclRunner *runner)
{
    return cacheKey(runner->deviceKey(), runner->buildOptions(), runner->source());
}


void pythonxm::OclCache::save(cl_program program, const std::string &fileName)
{
    size_t size = 0;
