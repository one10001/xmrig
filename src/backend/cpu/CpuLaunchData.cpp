...
pythonxm::CpuLaunchData::CpuLaunchData(const Miner *miner, const Algorithm &algorithm, const CpuConfig &config, const CpuThread &thread, size_t threads) :
    algorithm(algorithm),
...
bool pythonxm::CpuLaunchData::isEqual(const CpuLaunchData &other) const
{
...
pythonxm::CnHash::AlgoVariant pythonxm::CpuLaunchData::av() const
{
    if (intensity <= 2) {
...
const char *pythonxm::CpuLaunchData::tag()
{
    return cpu_tag();
}
