...
pythonxm::Benchmark::Benchmark(size_t workers, const IBackend *backend) :
    m_backend(backend),
    m_workers(workers)
{
}
...
bool pythonxm::Benchmark::finish(uint64_t totalHashCount)
{
...
void pythonxm::Benchmark::start()
{
    m_startTime = BenchState::start(m_workers, m_backend);
}


void pythonxm::Benchmark::printProgress() const
{
