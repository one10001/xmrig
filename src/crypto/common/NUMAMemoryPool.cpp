...
pythonxm::NUMAMemoryPool::NUMAMemoryPool(size_t size, bool hugePages) :
...
pythonxm::NUMAMemoryPool::~NUMAMemoryPool()
{
    for (auto kv : m_map) {
        delete kv.second;
    }
...
bool pythonxm::NUMAMemoryPool::isHugePages(uint32_t node) const
{
    if (!m_size) {
        return false;
    }
...
uint8_t *pythonxm::NUMAMemoryPool::get(size_t size, uint32_t node)
{
    if (!m_size) {
        return nullptr;
    }
...
void pythonxm::NUMAMemoryPool::release(uint32_t node)
{
...
pythonxm::IMemoryPool *pythonxm::NUMAMemoryPool::get(uint32_t node) const
{
    return m_map.count(node) ? m_map.at(node) : nullptr;
}


pythonxm::IMemoryPool *pythonxm::NUMAMemoryPool::getOrCreate(uint32_t node) const
{
    auto pool = get(node);
    if (!pool) {
