...
{
public:
    PYTHONXM_DISABLE_COPY_MOVE_DEFAULT(RxNUMAStoragePrivate)
...
pythonxm::RxNUMAStorage::RxNUMAStorage(const std::vector<uint32_t> &nodeset) :
    d_ptr(new RxNUMAStoragePrivate(nodeset))
{
}


pythonxm::RxNUMAStorage::~RxNUMAStorage()
{
    delete d_ptr;
}


bool pythonxm::RxNUMAStorage::isAllocated() const
{
    return d_ptr->isAllocated();
}


pythonxm::HugePagesInfo pythonxm::RxNUMAStorage::hugePages() const
{
...
pythonxm::RxDataset *pythonxm::RxNUMAStorage::dataset(const Job &job, uint32_t nodeId) const
{
...
void pythonxm::RxNUMAStorage::init(const RxSeed &seed, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode, int priority)
{
    d_ptr->setSeed(seed);
