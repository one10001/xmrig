...
pythonxm::RxCache::RxCache(bool hugePages, uint32_t nodeId)
{
...
pythonxm::RxCache::RxCache(uint8_t *memory)
{
    create(memory);
}


pythonxm::RxCache::~RxCache()
{
...
bool pythonxm::RxCache::init(const Buffer &seed)
{
    if (m_seed == seed) {
        return false;
    }
...
pythonxm::HugePagesInfo pythonxm::RxCache::hugePages() const
{
    return m_memory ? m_memory->hugePages() : HugePagesInfo();
}


void pythonxm::RxCache::create(uint8_t *memory)
{
    if (!memory) {
        return;
    }
