...
pythonxm::JsonChain::JsonChain() = default;


bool pythonxm::JsonChain::add(rapidjson::Document &&doc)
{
...
bool pythonxm::JsonChain::addFile(const char *fileName)
{
...
bool pythonxm::JsonChain::addRaw(const char *json)
{
    using namespace rapidjson;
    Document doc;
...
void pythonxm::JsonChain::dump(const char *fileName)
{
...
bool pythonxm::JsonChain::getBool(const char *key, bool defaultValue) const
{
...
const char *pythonxm::JsonChain::getString(const char *key, const char *defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
const rapidjson::Value &pythonxm::JsonChain::getArray(const char *key) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
const rapidjson::Value &pythonxm::JsonChain::getObject(const char *key) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
const rapidjson::Value &pythonxm::JsonChain::getValue(const char *key) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
        if (i != it->MemberEnd()) {
...
const rapidjson::Value &pythonxm::JsonChain::object() const
{
    assert(false);
...
double pythonxm::JsonChain::getDouble(const char *key, double defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
int pythonxm::JsonChain::getInt(const char *key, int defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
int64_t pythonxm::JsonChain::getInt64(const char *key, int64_t defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
pythonxm::String pythonxm::JsonChain::getString(const char *key, size_t maxSize) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
        if (i != it->MemberEnd() && i->value.IsString()) {
...
uint64_t pythonxm::JsonChain::getUint64(const char *key, uint64_t defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
...
unsigned pythonxm::JsonChain::getUint(const char *key, unsigned defaultValue) const
{
    for (auto it = m_chain.rbegin(); it != m_chain.rend(); ++it) {
        auto i = it->FindMember(key);
