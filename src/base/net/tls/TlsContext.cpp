...
pythonxm::TlsContext::~TlsContext()
{
    SSL_CTX_free(m_ctx);
}


pythonxm::TlsContext *pythonxm::TlsContext::create(const TlsConfig &config)
{
    if (!config.isEnabled()) {
        return nullptr;
    }
...
bool pythonxm::TlsContext::load(const TlsConfig &config)
{
...
bool pythonxm::TlsContext::setCiphers(const char *ciphers)
{
...
bool pythonxm::TlsContext::setCipherSuites(const char *ciphersuites)
{
...
bool pythonxm::TlsContext::setDH(const char *dhparam)
{
    DH *dh = nullptr;
...
void pythonxm::TlsContext::setProtocols(uint32_t protocols)
{
    if (protocols == 0) {
        return;
    }
