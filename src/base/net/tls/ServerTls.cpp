...
pythonxm::ServerTls::ServerTls(SSL_CTX *ctx) :
    m_ctx(ctx)
{
}


pythonxm::ServerTls::~ServerTls()
{
    if (m_ssl) {
        SSL_free(m_ssl);
    }
...
bool pythonxm::ServerTls::isHTTP(const char *data, size_t size)
{
    assert(size > 0);
...
bool pythonxm::ServerTls::isTLS(const char *data, size_t size)
{
    assert(size > 0);
...
bool pythonxm::ServerTls::send(const char *data, size_t size)
{
...
void pythonxm::ServerTls::read(const char *data, size_t size)
{
    if (!m_ssl) {
        m_ssl = SSL_new(m_ctx);
