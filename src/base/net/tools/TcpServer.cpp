...
static const pythonxm::String kLocalHost("127.0.0.1");


pythonxm::TcpServer::TcpServer(const String &host, uint16_t port, ITcpServerListener *listener) :
...
pythonxm::TcpServer::~TcpServer()
{
    Handle::close(m_tcp);
}


int pythonxm::TcpServer::bind()
{
    if (!m_version) {
...
void pythonxm::TcpServer::create(uv_stream_t *stream, int status)
{
    if (status < 0) {
        return;
    }
...
void pythonxm::TcpServer::onConnection(uv_stream_t *stream, int status)
{
