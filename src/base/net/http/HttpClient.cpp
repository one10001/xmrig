...
pythonxm::HttpClient::HttpClient(const char *tag, FetchRequest &&req, const std::weak_ptr<IHttpListener> &listener) :
...
bool pythonxm::HttpClient::connect()
{
    return m_dns->resolve(m_req.host);
}


void pythonxm::HttpClient::onResolved(const Dns &dns, int status)
{
    this->status = status;
...
void pythonxm::HttpClient::onTimer(const Timer *)
{
    close(UV_ETIMEDOUT);
}


void pythonxm::HttpClient::handshake()
{
...
void pythonxm::HttpClient::read(const char *data, size_t size)
{
...
void pythonxm::HttpClient::onConnect(uv_connect_t *req, int status)
{
