...
pythonxm::NetworkState::NetworkState(IStrategyListener *listener) : StrategyProxy(listener)
{
}
...
rapidjson::Value pythonxm::NetworkState::getResults(rapidjson::Document &doc, int version) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void pythonxm::NetworkState::printConnection() const
{
    if (!m_active) {
...
void pythonxm::NetworkState::printResults() const
{
    if (!m_hashes) {
...
const char *pythonxm::NetworkState::scaleDiff(uint64_t &diff)
{
...
std::string pythonxm::NetworkState::humanDiff(uint64_t diff)
{
...
void pythonxm::NetworkState::onActive(IStrategy *strategy, IClient *client)
{
...
void pythonxm::NetworkState::onJob(IStrategy *strategy, IClient *client, const Job &job, const rapidjson::Value &params)
{
...
void pythonxm::NetworkState::onPause(IStrategy *strategy)
{
...
void pythonxm::NetworkState::onResultAccepted(IStrategy *strategy, IClient *client, const SubmitResult &result, const char *error)
{
    add(result, error);
...
uint32_t pythonxm::NetworkState::latency() const
{
...
uint64_t pythonxm::NetworkState::avgTime() const
{
    if (m_latency.empty()) {
        return 0;
    }
...
uint64_t pythonxm::NetworkState::connectionTime() const
{
    return m_active ? ((Chrono::steadyMSecs() - m_connectionTime)) : 0;
}


void pythonxm::NetworkState::add(const SubmitResult &result, const char *error)
{
    if (error) {
        m_rejected++;
        return;
...
void pythonxm::NetworkState::stop()
{
    m_active      = false;
    m_diff        = 0;
    m_ip          = nullptr;
