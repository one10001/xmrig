...
pythonxm::Client::Tls::Tls(Client *client) :
    m_client(client)
{
...
bool pythonxm::Client::Tls::handshake()
{
    m_ssl = SSL_new(m_ctx);
...
bool pythonxm::Client::Tls::send(const char *data, size_t size)
{
...
const char *pythonxm::Client::Tls::fingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *pythonxm::Client::Tls::version() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void pythonxm::Client::Tls::read(const char *data, size_t size)
{
...
bool pythonxm::Client::Tls::send()
{
    return m_client->send(m_write);
}


bool pythonxm::Client::Tls::verify(X509 *cert)
{
    if (cert == nullptr) {
...
bool pythonxm::Client::Tls::verifyFingerprint(X509 *cert)
{
