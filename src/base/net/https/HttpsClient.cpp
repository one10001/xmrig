...
pythonxm::HttpsClient::HttpsClient(const char *tag, FetchRequest &&req, const std::weak_ptr<IHttpListener> &listener) :
...
pythonxm::HttpsClient::~HttpsClient()
{
    if (m_ctx) {
        SSL_CTX_free(m_ctx);
    }
...
const char *pythonxm::HttpsClient::tlsFingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *pythonxm::HttpsClient::tlsVersion() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void pythonxm::HttpsClient::handshake()
{
    m_ssl = SSL_new(m_ctx);
...
void pythonxm::HttpsClient::read(const char *data, size_t size)
{
...
void pythonxm::HttpsClient::write(std::string &&data, bool close)
{
...
bool pythonxm::HttpsClient::verify(X509 *cert)
{
    if (cert == nullptr) {
        return false;
    }
...
bool pythonxm::HttpsClient::verifyFingerprint(X509 *cert)
{
...
void pythonxm::HttpsClient::flush(bool close)
{
